...
export function useVueList({ props, globalOptions, filters, route, router, emit }) {
  ...
  async function requestData(addContext = {}) {
    error.value = false
  ...
  async function getData(contextParams) {
    await requestData(contextParams).then((response) => setData(response))
  }
